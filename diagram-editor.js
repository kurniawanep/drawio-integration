...
DiagramEditor.prototype.frameStyle =
  "position:absolute;border:0;width:100%;height:100%;";
...
//         fmt = 'xmlpng';
//     } else if (src.startsWith('data:image/svg+xml;') || elem.nodeName.toLowerCase() === 'svg') {
//         fmt = 'xmlsvg';
//     } else if (src.startsWith('http:') || src.startsWith('https:')) {
//         this.loadExternalFile(src, fmt); // New function to load external files
//         return this;
//     }
...
DiagramEditor.prototype.loadExternalFile = function (url, format) {
  fetch(url)
    .then((response) => response.text())
    .then((data) => {
      if (format === "xmlsvg" && url.endsWith(".svg")) {
        this.startEditing("data:image/svg+xml;base64," + btoa(data), format);
      } else if (format === "xmlpng" && url.endsWith(".png")) {
        this.startEditing("data:image/png;base64," + btoa(data), format);
      } else {
        this.startEditing(data, format); // For XML or other formats
      }
    })
...
DiagramEditor.prototype.handleMessage = function (msg) {
  if (msg.event == "configure") {
    ...
    this.xml = msg.xml;
    console.log("Current XML:", this.xml);
    alert("Copy the XML from the console log.");
